<?php session_start(); ?>
<html>
	<head><title>SubmitTask2</title></head>
	<body>
<?php	$host = "db.tecnico.ulisboa.pt"; $user = "ist178486"; $pass = "nhdd2783"; $dsn = "mysql:host=$host;dbname=$user";
		...
		$stmt = $connection->prepare("SELECT patient_id FROM patient WHERE name = :patient_name");
		$stmt->bindParam(':patient_name', $patient_name);
		$stmt->execute();
		$data = $stmt->fetchAll(PDO::FETCH_COLUMN,0);	$patient_id = $data[0];
		$app_date = $_REQUEST['app_date'];

		if( date('w', strtotime($app_date)) % 6 == 0 ) {
			echo("Nao ha consultas ao fim-de-semana."); 
?> 			<p><form action="Task2.php" method="post"><input type="submit" value="Remarcar consulta"/></form>
